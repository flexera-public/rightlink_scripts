...
# Grab info from policy file
policy_username=`grep -E "^$username:|^[0-9a-z_\-]*:$username:" /var/lib/rightlink/login_policy` || true
if [[ "$policy_username" == "" ]]; then
  # Not in policy file
  exit 0
fi
read preferred_name unique_name policy_uid <<< $(echo $policy_username | cut -d: -f1,2,4 --output-delimiter=' ')

# Grab info from system
system_username=`getent passwd ${username}` || true
...
read system_uid <<< $(echo $system_username | cut -d: -f3 --output-delimiter=' ')
...
if [[ "$system_uid" == "$policy_uid" ]]; then
  ...
  logger -i -p auth.info -t rs-ssh-keys.sh "username '${username}' matches entry in login policy"
  echo $policy_username | cut -d: -f7- | tr : "\n"
else
